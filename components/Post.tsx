...
export default function Post({ post }: PostProps) {
  ...
  useEffect(() => {
    ...
    const down = post.downvotes || []
    // Backward compat: old posts have likes[] instead of upvotes[]
    const legacyUp = (post as any).likes || []
    const allUp = up.length > 0 ? up : legacyUp
    setScore(allUp.length - down.length)
    if (user) {
      if (allUp.includes(user.uid)) setVoteState('up')
      else if (down.includes(user.uid)) setVoteState('down')
      else setVoteState(null)
    }
  }, [post.upvotes, post.downvotes, (post as any).likes, user])
    ...
    try {
      if (voteState === direction) {
        ...
        await updateDoc(postRef, {
          [direction === 'up' ? 'upvotes' : 'downvotes']: arrayRemove(user.uid),
        })
        ...
        if (voteState) {
          await updateDoc(postRef, {
            [voteState === 'up' ? 'upvotes' : 'downvotes']: arrayRemove(user.uid),
          })
        }
        await updateDoc(postRef, {
          [direction === 'up' ? 'upvotes' : 'downvotes']: arrayUnion(user.uid),
        })
  ...
  return (
    <div className="post-card cursor-pointer flex gap-2.5" style={borderStyle}>
      {/* Vote column */}
      <div className="flex flex-col items-center gap-0.5 pt-0.5 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
        <button
        ...
        <span className={`text-xs font-bold tabular-nums ${voteState === 'up' ? 'text-primary' : voteState === 'down' ? 'text-rose-400' : 'text-slate-400'}`}>
          {score}
        </span>
        <button
      ...
      {/* Content column */}
      <div className="flex-1 min-w-0" onClick={() => router.push(`/post/${post.id}`)}>
        {/* Header */}
        <div className="flex items-center gap-1.5 mb-1.5">
          ...
          >
            {post.userPhoto ? (
              <img src={post.userPhoto} alt={post.userName} className="w-6 h-6 rounded-full object-cover avatar-ring" />
            ) : (
              <div className="w-6 h-6 rounded-full bg-gradient-to-br from-primary via-accent to-secondary flex items-center justify-center text-white font-semibold text-[9px] avatar-ring">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); router.push(`/profile/${post.userId}`) }}
            className="font-semibold text-xs text-slate-200 hover:text-primary transition-colors truncate"
          >
            {post.userName}
          </button>
          {post.category && categoryStyle && (
            <span className="text-[9px] px-1.5 py-px rounded-full font-medium" style={badgeStyle}>
              {categoryStyle.name}
            </span>
          )}
        ...
        {(!hasArticle || (post.content && post.content !== post.articleTitle)) && (
          <p className="text-slate-300 text-sm leading-relaxed line-clamp-3 mb-1.5">{post.content}</p>
        )}
            ...
            {post.articleImage ? (
              <div className="relative">
                <div className="w-full h-36 sm:h-44 overflow-hidden bg-slate-900">
                  <img src={post.articleImage} alt="" className="w-full h-full object-cover group-hover/link:scale-105 transition-transform duration-300"
                    onError={(e) => { (e.target as HTMLImageElement).parentElement!.style.display = 'none' }} />
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-3">
                  <h4 className="text-sm font-semibold text-white leading-snug line-clamp-2 drop-shadow-lg">{post.articleTitle}</h4>
                  <div className="flex items-center gap-1.5 mt-1.5">
                    {faviconUrl && <img src={faviconUrl} alt="" className="w-3 h-3 rounded-sm opacity-70"
                      onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} />}
              ...
              <div className="flex items-center gap-3 p-3 bg-white/[0.02]">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-medium text-slate-200 group-hover/link:text-primary transition-colors leading-snug line-clamp-2">{post.articleTitle}</h4>
        ...
        <div className="flex items-center gap-1.5 pt-1.5 text-slate-500">
          ...
          <span className="text-xs font-medium">{post.commentCount || 0} comments</span>
        </div>
      </div>
    </div>
  )
